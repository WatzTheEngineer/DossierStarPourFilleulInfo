...
def decrytion(filepath: str) -> str:
    ...
    with open(filepath, "rb") as rfile:
        ...
        while rfile.peek():
            ...
            b = bytes([unicode ^ key[
                (i % len(key))]])  # On XOR le caractère avec le rang de la clé égal à i modulo sa longeur
            try:
    ...
    if error:
        print(
            TEXTFORM.ERROR +
            "Le fichier de sauvegarde est endommagé, une réparation va être effectuée" +
            TEXTFORM.DEFAULT
        )  # Réparation du fichier si il est illisible
        repair()
