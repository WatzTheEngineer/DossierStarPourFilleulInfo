...
def tourPuissance4(joueur1: Joueur, joueur2: Joueur):
    ...
    pion_j: str = TEXTCOLOR.YELLOW + 'O' + TEXTCOLOR.DEFAULT
    pion_r: str = TEXTCOLOR.RED + 'O' + TEXTCOLOR.DEFAULT
    aligne: bool = False

    mrPropre()
    print(joueur1.pseudo, "joueur avec les", pion_j)  # On indique à chaue joueur ses pions
    print(joueur2.pseudo, "joueur avec les", pion_r)
    ...
    while not aligne and not plateauPlein(plateau):  # On joue tant qu'il n'y a ni égalité ni victoire
        ...
        placerPion(plateau, pion_j)
        mrPropre()
        afficherPlateau(plateau)
        aligne = pionAligne(plateau, pion_j)
        if aligne:
            print(TEXTCOLOR.GREEN + "Le joueur " + joueur1.pseudo + " a gagné !" + TEXTCOLOR.DEFAULT + "\n")
            incrementScore(joueur1, "puissance 4")
        if plateauPlein(plateau) or aligne:
            break

        print(joueur2.pseudo + ", joue !")
        placerPion(plateau, pion_r)
        mrPropre()
        afficherPlateau(plateau)
        aligne = pionAligne(plateau, pion_r)
        if aligne:
            print(TEXTCOLOR.GREEN + "Le joueur " + joueur2.pseudo + " a gagné !" + TEXTCOLOR.DEFAULT + "\n")
